...
public static class TaskLifeCycleHelper
{
                                              ...
                                              CancellationToken        cancellationToken)
  {
    ...
    {
      var orderedAndGroupedMessages = OrderAndGroupMessageData(messages);
      foreach (var group in orderedAndGroupedMessages)
      {
  ...
  internal static IEnumerable<IGrouping<(string PartitionId, int Priority), MessageData>> OrderAndGroupMessageData(IEnumerable<MessageData> messages)
    => messages.GroupBy(msg => (msg.Options.PartitionId, msg.Options.Priority));
                                                    ...
                                                    CancellationToken cancellationToken = default)
  {
    ...
    var dataMessages = taskTable.FindTasksAsync(data => data.SessionId == sessionId && data.Status == TaskStatus.Paused,
                                                data => new MessageData(data.TaskId,
                                                                        data.SessionId,
                                                                        data.Options),
                                                cancellationToken);

    // var orderedAndGroupDataMessages = OrderAndGroupMessageData(dataMessages);
    await foreach (var grouping in dataMessages.GroupBy(msg => (msg.Options.PartitionId, msg.Options.Priority))
                                               // TODO: Handle the messageData Priority
                                               // .OrderByDescending()
